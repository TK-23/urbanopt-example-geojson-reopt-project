...
df <- read.csv('/Users/tkwasnik/github/urbanopt-example-geojson-reopt-project/compiled_SDGE_results.csv')
df$scenario <- gsub('run/','',df$scenario)
...
ggplot(df, aes(x=scenario,y=storage_size_kwh)) + geom_col() + ylab("Storage (kWh)") + xlab("") +  facet_wrap(df$level)+ theme.main + theme.tilt_x
